...
{
	ifstream input;
	input.open("Data//Txt//Residency_Profile.txt", ios::in);
	if (!input.is_open())
	{
	...
	input.open("Data//Txt//Admission_Profile.txt", ios::in);
	if (!input.is_open())
	{
		//throw "Unable to open file Student";
	}
...
void Management_Profile::Write_File()
{
	...
	ofstream output;
	output.open("Data//Txt//Residency_Profile.txt", ios::out);
	while (p != nullptr)
	{
	...
	output.close();
	output.open("Data//Txt//Admission_Profile.txt", ios::out);
	while (p1 != nullptr)
	{
