...
{
    ifstream input;
    input.open("Data//Txt//Room.txt", ios::in);
    if (!input.is_open())
    {
...
{
    ofstream output;
    output.open("Data//Txt//Room.txt", ios::out);
...
void Mamagement_Room::Sort(Node<Room> *p, Doubly_Linked_List<Room>& Data_Room)
{
    Xoa_o(4, 14, 39, 35);
    ...
    if (a == 0)
    {
        if (b == 0) Data_Room.Selection_Sort(p, Sort_RC_DES);
        else  Data_Room.Selection_Sort(p ,Sort_RC_ASC);
    }
    else
    {
        if (b == 0) Data_Room.Selection_Sort(p,Sort_NS_DES);
        else  Data_Room.Selection_Sort(p, Sort_NS_ASC);
    }
}