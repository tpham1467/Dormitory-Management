...
{
    ofstream output;
    output.open("Data//Txt//Student.txt", ios::out);
...
{
    ifstream input;
    input.open("Data//Txt//Student.txt", ios::in);
    if (!input.is_open())
    {
...
void Management_Student::Menu_Student(bool flag)
{
    ...
    ds:
        ...
        if (index == -3)
        {
            Sort(Database.Get_P_Head(),Database);
            Sap_Xep_Xong();
        ...
        find:
            ...
            else
            {
                ...
                {
                    Sort(_data_find.Get_P_Head(),_data_find);
                    Sap_Xep_Xong();
                    Hien_thi_danh_sach(arr_1, 4);
                    goto sort;
                }
...
void Management_Student::Sort(Node<Student>* p, Doubly_Linked_List<Student>& Data_Student)
{
        Xoa_o(4, 14, 39, 35);
        ...
        if (a == 0)
        {
            if (b == 0) Data_Student.Selection_Sort(Database.Get_P_Head(), Sort_Name_DES);
            else  Data_Student.Selection_Sort(p, Sort_Name_ASC);
        }
        else
        {
            if (b == 0) Data_Student.Selection_Sort(p, Sort_MSSV_DES);
            else  Data_Student.Selection_Sort(p, Sort_MSSV_ASC);
        }
}